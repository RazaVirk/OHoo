/* ===== OHOO TAP - SCRIPT.JS ===== */
/* PART 3/5: Core + Auth + Lock + Tap + Power-ups */

// ===== FIREBASE CONFIG (baad mein) =====
const FIREBASE_CONFIG = {
    // apiKey: "YOUR_API_KEY",
    // authDomain: "ohoo-tap.firebaseapp.com",
    // projectId: "ohoo-tap",
    // storageBucket: "ohoo-tap.appspot.com",
...
// ===== FEATURE LOCKS =====
const FEATURE_LOCKS = {
    premium: { requiresPayment: true, price: 500 }
};
for (let i = 1; i <= 10; i++) {
    FEATURE_LOCKS[`autobot-level-${i}`] = { requiresPayment: true, price: i * 100 };
}
...
const STORAGE_KEY = "ohoo_tap_user";
...
function canUseFeature(featureKey) {
    const lock = FEATURE_LOCKS[featureKey];
    if (!lock) return { allowed: true };
    if (lock.requiresPayment) {
        if (gameState.purchasedFeatures && gameState.purchasedFeatures.includes(featureKey)) return { allowed: true };
        return { allowed: false, reason: `Yeh feature kharidna parega (${lock.price} PKR)`, showPayment: true, price: lock.price };
    }
    return { allowed: true };
}
...
function login() {
    ...
    if (!user || user.password !== btoa(password)) return showToast("Galat username ya password!", "error");
    currentUser = username;
    gameState = { ...gameState, ...user.data };
...
function signup() {
    ...
    if (password.length < 4) return showToast("Password min 4 chars!", "error");
    const users = JSON.parse(localStorage.getItem(USERS_KEY) || "{}");
    ...
    const newUser = {
        ...
        data: {
            ...gameState,
            username, referralCode: refCode, referredBy: referral || null,
            coins: 0, energy: MAX_ENERGY,
            lastLogin: Date.now(), lastOnline: Date.now(),
            premium: false, premiumUntil: 0, autobotLevel: 0,
            purchasedFeatures: [],
            ownedAvatars: ["🚀", "👾", "🐱", "🦊", "🐼", "🐸"],
            ownedFrames: [], ownedTapEffects: ["default"],
            level: 1, lifetimeCoins: 0, taps: 0, totalTaps: 0,
            withdrawHistory: [], levelBonusHistory: [], notifications: [],
            achievements: {}, luckyBoxHistory: [],
            lastSpinTime: 0, lastScratchTime: 0,
            lastAutoAdTime: Date.now(), coinsSinceLastAd: 0,
            adVideosWatched: 0, adTotalEarned: 0
        }
    ...
    if (referral) {
        const referrer = Object.keys(users).find(u => users[u].data.referralCode === referral);
        if (referrer) {
            users[referrer].data.coins += 50000;
            users[referrer].data.referrals = (users[referrer].data.referrals || 0) + 1;
            users[referrer].data.activeReferrals = (users[referrer].data.activeReferrals || 0) + 1;
            users[referrer].data.referralEarned = (users[referrer].data.referralEarned || 0) + 50000;
        }
        newUser.data.coins += 25000;
    }

    users[username] = newUser;
    localStorage.setItem(USERS_KEY, JSON.stringify(users));
    currentUser = username;
    gameState = newUser.data;
    localStorage.setItem(STORAGE_KEY, username);
    enterGame();
...
function checkAutoLogin() {
    ...
    currentUser = saved;
    gameState = { ...gameState, ...users[saved].data };
    calculateOfflineEarnings();
    enterGame();
}
...
function enterGame() {
    ...
    if (gameState.premium === undefined) gameState.premium = false;
    if (gameState.autobotLevel === undefined) gameState.autobotLevel = 0;
    ...
    if (gameState.lastSpinTime === undefined) gameState.lastSpinTime = 0;
    if (gameState.lastScratchTime === undefined) gameState.lastScratchTime = 0;
    if (gameState.lastAutoAdTime === undefined) gameState.lastAutoAdTime = Date.now();
    if (gameState.coinsSinceLastAd === undefined) gameState.coinsSinceLastAd = 0;
    if (gameState.adVideosWatched === undefined) gameState.adVideosWatched = 0;
    if (gameState.adTotalEarned === undefined) gameState.adTotalEarned = 0;
...
function calculateOfflineEarnings() {
    if (!gameState.lastOnline) return;
    const now = Date.now();
    const hours = Math.min((now - gameState.lastOnline) / (1000 * 60 * 60), 8);
    if (hours < 0.1) return;
    let rate = 10000;
    if (gameState.autobotLevel > 0) rate += gameState.autobotLevel * 5000;
    if (gameState.premium) rate *= 10;
    const earned = Math.max(MIN_OFFLINE_COINS, Math.floor(rate * hours));
    setTimeout(() => {
        showToast(`Welcome back! Offline ${formatNum(earned)} coins kamaye!`, "success");
...
function showTerms() { document.getElementById("terms-modal").classList.remove("hidden"); }
...
function activatePowerUp(type) {
    ...
    const msgs = {
        double: `🔥 2x Coins! (30 sec) -${formatNum(cost)} coins`,
        infinite: `♾️ Infinite Energy! (1 min) -${formatNum(cost)} coins`,
        autotap: `🤖 Auto-Tap! (5 min) -${formatNum(cost)} coins`,
        megatap: `💥 Mega Tap! (1 min) -${formatNum(cost)} coins`
    };
...
/* ===== PART 4/5: Rewards + Missions + Lucky Box + Avatars + Shop ===== */

// ===== SPIN WHEEL (Direct 10k + 4hr cooldown) =====
function spinWheel() {
    const now = Date.now();
...
// ===== SCRATCH CARD (Direct 10k + 4hr cooldown) =====
...
function generateMissions() {
    ...
    if (gameState.lastMissionDate === today && gameState.missions.length > 0) return;
    gameState.missions = [
...
function renderMissions() {
    ...
    if (!container) return;
    if (!gameState.missions.length) generateMissions();
...
function updateLuckyBoxTimer() {
    ...
    if (!el) return;
    const now = Date.now();
    const next = gameState.luckyBoxLastOpen + LUCKYBOX_COOLDOWN;
    if (now >= next) {
...
function openLuckyBox() {
    const now = Date.now();
    if (now < gameState.luckyBoxLastOpen + LUCKYBOX_COOLDOWN) return showToast("Box ready nahi hai!", "warning");
    const rand = Math.random();
...
function unlockUpgrade(level) {
    const featureKey = `autobot-level-${level}`;
    if (!gameState.purchasedFeatures) gameState.purchasedFeatures = [];
    if (!gameState.purchasedFeatures.includes(featureKey)) gameState.purchasedFeatures.push(featureKey);
    gameState.autobotLevel = Math.max(gameState.autobotLevel || 0, level);
    showToast(`✅ Auto-Bot Level ${level} unlocked! ${level * AUTOBOT_COINS_PER_LEVEL} coins/sec`, "success");
    confetti({ particleCount: 150 });
    saveUser(); renderUpgrades(); updateUI();
}
...
function renderLeaderboard() {
    ...
    if (!container) return;
    const users = JSON.parse(localStorage.getItem(USERS_KEY) || "{}");
    let arr = Object.keys(users).map(u => ({
        username: u, avatar: users[u].data.avatar || "🚀",
        coins: users[u].data.coins || 0, taps: users[u].data.totalTaps || 0, refs: users[u].data.referrals || 0,
        isYou: u === currentUser
    }));
...
/* ===== PART 5/5: Social + Chat + Watch & Earn + Auto Ad + Modals + Admin ===== */
...
function searchFriend() {
    ...
    const users = JSON.parse(localStorage.getItem(USERS_KEY) || "{}");
    if (!users[q]) return showToast("User nahi mila!", "error");
...
function renderFriends() {
    ...
    container.innerHTML = gameState.friends.map(f => `
        <div class="flex items-center gap-2 bg-slate-800/60 p-2 rounded-lg">
            <span class="text-xl">${users[f]?.data?.avatar || "🚀"}</span>
...
function copyReferralLink() {
    const link = `https://ohootap.com/signup?ref=${gameState.referralCode}`;
    ...
    container.innerHTML = milestones.map(m => {
        ...
        return `
            <div class="flex justify-between items-center bg-slate-800/60 p-3 rounded-xl">
                ...
                <span class="text-xs font-black ${done ? "text-emerald-400" : "text-slate-500"}">${done ? "✓ DONE" : `${gameState.referrals}/${m.count}`}</span>
            </div>
        `;
    }).join("");
}
...
function resetAdUI() {
    document.getElementById("ad-video-placeholder").classList.remove("hidden");
...
function startAd() {
    ...
    document.getElementById("ad-video-placeholder").classList.add("hidden");
...
function finishAutoAd() {
    ...
    document.getElementById("auto-ad-modal").classList.add("hidden");
    saveUser();
}
...
window.addEventListener("beforeunload", () => {
    if (currentUser) { gameState.lastOnline = Date.now(); saveUser(); }
});

// ===== ADMIN FUNCTIONS =====
function adminActivatePremium(username) {
    const users = JSON.parse(localStorage.getItem(USERS_KEY) || "{}");
    ...
    if (!users[username].data.purchasedFeatures.includes("premium")) users[username].data.purchasedFeatures.push("premium");
    users[username].data.notifications.unshift({ text: "👑 Premium activated!", time: Date.now(), read: false });
    localStorage.setItem(USERS_KEY, JSON.stringify(users));
    showToast(`${username} ko Premium diya!`, "success");
}

function adminUnlockUpgrade(username, level) {
    const users = JSON.parse(localStorage.getItem(USERS_KEY) || "{}");
    if (!users[username]) return;
    const key = `autobot-level-${level}`;
    if (!users[username].data.purchasedFeatures) users[username].data.purchasedFeatures = [];
    if (!users[username].data.purchasedFeatures.includes(key)) users[username].data.purchasedFeatures.push(key);
    users[username].data.autobotLevel = Math.max(users[username].data.autobotLevel || 0, level);
    users[username].data.notifications.unshift({ text: `✅ Auto-Bot Level ${level} unlocked!`, time: Date.now(), read: false });
    localStorage.setItem(USERS_KEY, JSON.stringify(users));
    showToast(`${username} ko Level ${level} diya!`, "success");
}

function adminApproveWithdraw(username, withdrawId) {
    const users = JSON.parse(localStorage.getItem(USERS_KEY) || "{}");
    if (!users[username]) return;
    const w = users[username].data.withdrawHistory.find(x => x.id === withdrawId);
    if (w) {
        w.status = "Completed";
        users[username].data.notifications.unshift({ text: `💸 Withdrawal ${w.pkr} PKR completed!`, time: Date.now(), read: false });
        localStorage.setItem(USERS_KEY, JSON.stringify(users));
...
function openAdminPanel() {
    ...
    if (pass !== "admin123") return showToast("Wrong password!", "error");
